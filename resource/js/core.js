	...
	this._loadCss = function() {
		...
		var tempMethod = function(url) {
			...
			if(flag == 0) {
				// 未载入过
				temp[temp.length] = url;
				var css = '<link href="'+THEME_URL+'box.css" rel="stylesheet" type="text/css">';
				$('head').append(css);
			}
		};
		// 返回内部包函数,供外部调用并可以更改temp的值
...
//    return flase;
    if(arguments.length > 0) {
		...
		var file = THEME_URL + '/js/plugins/core.' + arg[0] + '.js';
...
var ui = {
	...
	showblackout: function() {
		if($('.boxy-modal-blackout').length > 0) {
    		...
    		$('<div class ="boxy-modal-blackout"></div>').appendTo("body");  //.animate({opacity: .3},100)
        	$(".boxy-modal-blackout").css({
        		//暂时加60;
        	    height:height+60+'px',width:$('body').width()+'px',zIndex: 991, opacity: .3
        	})
    	}
	},
	/**
	...
	confirm: function(o, text, _callback) {
		...
		this.html = '<div id="ts_ui_confirm" class="ts_confirm"><div class="layer-mini-info"><dl><dt class="txt"> </dt><dd class="action"><a class="cancelBtn ml20" href="javascript:void(0)"><span>取消</span></a><a class="btn btn_solidBlue ml10 btn_ok" href="javascript:void(0)"><span>确定</span></a></dd></dl></div></div>';
		...
		$("#ts_ui_confirm .smallHeadFont").one('click',function(){
	...
	confirmBox: function(title, text, _callback) {
		...
		var content = '<div class="pop-create-group"><dl><dt class="txt">'+ text + '</dt><dd class="action"><a class="cancelBtn mr20"  id="cancel" href="javascript:void(0)""><span>取消</span></a><a class="trueBtn" href="javascript:void(0)"><span>确定</span></a></dd></dl></div>';
		...
		var _this = this;
		$("#tsbox .smallHeadFont").one('click',function(){
		...
		//确定执行事件
		$("#tsbox .trueBtn").on('click',function(){
			if("undefined" == typeof(callback)){
				return true;
			}else{
				if("function"==typeof(callback)){
	...
	box: {
		WRAPPER: '<div class="wrap-layer" id="tsbox" style="display:none">\
     			  <div class="content-layer wrap-layer_bg">\
		...
		init: function(title, callback) {
			...
			if(title!=" "){  //有标题时头部高度为60 加上padding值 20150105 yuxuemei添加
               $(".hd").addClass('hdPadd borderB1 hd_bg');
			}
		 ...
		 * @return void
		 */
		load:function(requestUrl,title,callbacka,requestData,type) {
			$('#tsbox').remove();
...
function urlParamDel(url,name){
	var n=url.indexOf("?");
	if(n>=0){
		var url_b=url.substr(n+1);
		var url_a=url.substr(0,n+1);
		var reg=new RegExp("(^|&)" + name + "=([^&]*)(&|$)", "i");
        var url_c=url_a+url_b.replace(reg,"");
		var ns=url_c.indexOf("?")+1;
		if(ns==url_c.length){
		  url_c=url.substr(0,ns-1);
		}
		return url_c;
	}else{
	    return url;
